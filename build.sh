...
if [[ $UNAME == *"MINGW"* ]]; then
  CC=clang CXX=clang++ cmake -G "MSYS Makefiles" -DVEC4_OPT=On -DCRC_OPT=On -DMUPENPLUSAPI=On ../../src/
else
...
if [[ $UNAME == *"MINGW"* ]]; then
  ...
  sed -i 's/python /python3 /g' core.vcxproj
  if [[ $UNAME == *"MINGW64"* ]]; then
  ...
  cp $base_dir/angrylion-rdp-plus/msvc/build/Release/mupen64plus-video-angrylion-plus.dll $install_dir
else
